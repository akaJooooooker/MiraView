...
目前版本：**0.4.3 主視窗整合式 RTX Video VSR／HDR 顯示**。
...
- 右鍵選單的「RTX HDR 模式」可直接開關整合式 VSR + HDR，並選擇標準、鮮明或柔和模式。
- 「說明 → 語言」可在繁體中文與 English 之間切換；主視窗、右鍵選單、提示及 RTX 視窗會跟隨並記住選擇。

## 執行

一般使用者可從 [GitHub Releases](https://github.com/akaJooooooker/MiraView/releases/latest) 下載 `MiraView-0.4.3-win-x64.zip`，完整解壓縮後直接執行 `MiraView.exe`。請保留 `nvngx_vsr.dll` 與 `nvngx_truehdr.dll` 在執行檔旁，RTX 功能才可使用；0.4.3 不需要 `MiraViewHdrPreview.exe`。壓縮檔內附中英雙語 `README.txt`。
...
| 右鍵 | 功能選單；「RTX HDR 模式」可開關 HDR 並選擇標準／鮮明／柔和 |
...
- VSR 需要 `nvngx_vsr.dll`；整合 HDR 顯示另需 `nvngx_truehdr.dll`。0.4.3 的 HDR10 swap chain 已直接內建於 `MiraView.exe`。
