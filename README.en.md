...
Current version: **0.4.0 image-viewing core with integrated RTX Video VSR/HDR presentation**.
...
- Press `H` to open the integrated VSR Ultra → TrueHDR D3D11 GPU pipeline, presented directly through a 10-bit HDR10/Rec.2020 output with no CPU readback between VSR and HDR.
...
- The HDR viewer indexes the same folder and preloads four neighboring images in each direction. Navigate continuously with the arrow keys, Page Up/Page Down, Space, or the mouse wheel; each image receives the required VSR/TrueHDR rendering.
- Multiple MiraView or RTX presentation windows may run at the same time, with an independent NGX instance per process. Two concurrent VSR Ultra → TrueHDR pipelines have been verified locally on one RTX 4070 Ti SUPER; actual capacity and performance depend on GPU load and VRAM.
...
Download `MiraView-0.4.0-win-x64.zip` from [GitHub Releases](https://github.com/akaJooooooker/MiraView/releases/latest), extract the complete archive, and run `MiraView.exe`. Keep `MiraViewHdrPreview.exe`, `nvngx_vsr.dll`, and `nvngx_truehdr.dll` next to the main executable for RTX features. The archive also contains a bilingual `README.txt`.
...
| `H` | Open “RTX Video Enhancement (VSR + HDR)”; use arrows/wheel to navigate, `M` to maximize, `F11` for fullscreen, and `1`/`2`/`3` for intensity presets |
...
- VSR requires `nvngx_vsr.dll`. Integrated HDR presentation additionally requires `MiraViewHdrPreview.exe` and `nvngx_truehdr.dll`.
...
If the computer lacks a compatible RTX GPU, uses an outdated driver, has HDR disabled in Windows or on the display, or is missing a required DLL, MiraView displays an error dialog and falls back to normal image viewing instead of closing. HDR runs in a separate process, so an HDR initialization failure does not take down the main viewer; the background VSR worker also catches processing exceptions.
...
- RTX VSR and TrueHDR are connected as GPU textures on the same D3D11 device. The 10-bit HDR swap chain still uses the separate integrated-presentation window, while the main viewer remains SDR.
- The primary v0.4.1 goal is to integrate the HDR10 swap chain into the original MiraView window instead of opening a separate RTX window.
